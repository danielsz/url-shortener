(ns url-shortener.shortener
  (:require [taoensso.carmine :as redis]
            ...
            [url-shortener.schema :refer [group-links-key default-group-id group-key owner-groups-key targets-key]]
            [ring.util.response :refer [response bad-request not-found redirect content-type]]))

(defn shorten! [{{:keys [url owner-id group-id description target]
                  :or   {owner-id "anonymous" description ""}} :params}]
  (let [path     (hash-url url owner-id)
    ...
    (redis/wcar nil
                ...
                (when (seq targets)
                  (apply redis/sadd (targets-key path) targets))
...
(defn shorten [{{:keys [url]} :params :as req}]
  (if (.isValid url-validator url)
    (-> (response (str (System/getProperty "shortener.service") (shorten! req)))
...
(defn handle-redirect [{redis :redis pubsub :pubsub} {{path :path} :path-params remote-addr :remote-addr headers :headers :as request}]
  (let [referer (get headers "referer")        
        [rc url group-id owner-id] (redis/wcar nil
                                               (redis/hincrby path "clicks" 1)
                                               (redis/hget    path "url")
                                               (redis/hget    path "group-id")
                                               (redis/hget   path "owner-id"))]
    (log/debug "rc" rc)
    (when url
      (a/thread (a/>!! (:channel pubsub) {:topic :click :path path :owner-id owner-id :group-id group-id :remote-addr remote-addr :referer referer})))
    (if url
      (redirect url)
...
(defn handle-redirect-with-legacy [{redis :redis pubsub :pubsub} {{path :path} :path-params remote-addr :remote-addr headers :headers :as request}]
  (let [referer (get headers "referer")        
    ...
    (cond
      (= type "hash")
      (let [[_ url group-id owner-id] (redis/wcar nil
                                                  (redis/hincrby path "clicks" 1)
                                                  (redis/hget    path "url")
                                                  (redis/hget   path "group-id")
                                                  (redis/hget   path "owner-id"))]
