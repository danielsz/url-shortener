(ns url-shortener.analytics
  (:require [url-shortener.shared.utils :refer [ip-validator epoch-now today this-week this-month guest?]]
            [url-shortener.schema :refer [ips-key referrers-key daily-key group-key TTL-ANALYTICS TTL-GUEST-ANALYTICS group-ips-key group-daily-key countries-key group-countries-key weekly-key monthly-key group-monthly-key group-weekly-key platforms-key group-platforms-key platforms-daily-key group-platforms-daily-key]]
...
(defn write-country [geoip path group-id remote-addr ttl]
        ...
        (let [country (.get m "country_code")]
          (when country
            (redis/wcar nil
              ...
              (redis/expire  (countries-key path) ttl)
              (redis/hincrby (group-countries-key group-id) country 1)
              (redis/expire  (group-countries-key group-id) ttl))))))
      ...
      (let [platform (referrer->platform referer)
        ...
        (when referer
          (redis/wcar nil
                      ...
                      (redis/expire (referrers-key path) ttl)))))
    (catch Exception e
